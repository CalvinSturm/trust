...
#[test]
fn toolfw_enforces_client_id_policy_without_upstream_contact() {
    ...
    write_json_line(
        &mut stdin,
        &json!({"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"views.query","arguments":{"view":"notes_recent"},"auth":{"token":bob}}}),
    );
    ...
    assert!(allowed.get("error").is_none() || allowed["error"].is_null());
