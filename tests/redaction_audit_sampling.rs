...
#[test]
fn audit_samples_are_redacted_when_enabled() {
    ...
    fs::write(
        &redact,
        "patterns:\n  - name: sentinel\n    regex: \"hello world secret\"\n    replacement: \"REDACTED-SENTINEL\"\n",
    )
    .unwrap();
    ...
    let mut child =
        if let (Some(toolfw), Some(fake)) = (bin_path("toolfw"), bin_path("fake-upstream")) {
            Command::new(toolfw)
                ...
                .arg("--redact")
                .arg(&redact)
                .arg("--audit-payload-sample-bytes")
                .arg("512")
                .arg("--")
                .arg(fake)
                .arg("--log")
    ...
    write_json_line(
        &mut stdin,
        &json!({"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"files.write","arguments":{"mount":"notes","path":"x.txt","content":"hello world secret"}}}),
    );
    ...
    assert!(!audit_txt.contains("hello world secret"));
    assert!(audit_txt.contains("REDACTED-SENTINEL"));
}
